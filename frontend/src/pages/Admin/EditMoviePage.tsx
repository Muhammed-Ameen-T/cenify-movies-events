...
const MOVIE_UPDATED_KEY = 'movieEditSubmitted';
...
const EditMovieForm: React.FC = () => {
  ...
  const [formSubmitted, setFormSubmitted] = useState<boolean>(() => {
    return localStorage.getItem(MOVIE_UPDATED_KEY) === 'true';
  });
  ...
  const { data: movie, isLoading, error } = useQuery({
    queryKey: ['movie', id],
    queryFn: () => movieService.findById(id!),
    enabled: !!id,
  });
  ...
  const handleClearForm = () => {
    localStorage.removeItem(MOVIE_UPDATED_KEY);
          ...
          <AnimatePresence mode="wait">
            {formSubmitted ? (
              ...
              >
                ...
                >
                  Your movie has been updated. You can manage it from the dashboard.
                </motion.p>
                <motion.button
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                ...
                <div className="p-6 sm:p-8">
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div>
                          <input
                            ...
                            min="0"
                            max="59"
                            className={`w-full py-3 px-Montserrat rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 focus:outline-none ${
                    ...
                    <motion.div variants={itemVariants} className="pt-4">
                      <motion.button
                        type="submit"
                        disabled={isSubmitting || !watchedPoster || updateMovieMutation.isPending}
                        className={`w-full py-4 px-4 rounded-lg font-semibold text-white transition duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500 ${
                          isSubmitting || !watchedPoster || updateMovieMutation.isPending
