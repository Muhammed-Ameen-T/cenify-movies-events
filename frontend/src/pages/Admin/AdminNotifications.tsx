...
const AdminNotifications: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['admin-notifications', currentLimit, currentFilter],
    queryFn: async () => {
  ...
  const markAsReadMutation = useMutation<void, Error, string>({
    ...
    onMutate: async (notificationId) => {
      await queryClient.cancelQueries({ queryKey: ['admin-notifications', currentLimit, currentFilter] });
      const previousData = queryClient.getQueryData(['admin-notifications', currentLimit, currentFilter]);
      queryClient.setQueryData(['admin-notifications', currentLimit, currentFilter], (oldData: any) => {
        if (!oldData) return oldData;
        return {
          ...oldData,
          notifications: oldData.notifications.map((notification: AdminNotification) =>
    ...
    onError: (err, _, context) => {
      queryClient.setQueryData(['admin-notifications', currentLimit, currentFilter], context?.previousData);
    ...
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-notifications', currentLimit, currentFilter] });
    },
  });
  ...
  if (isError) {
    return (
      <div className="bg-transparent px-10 mt-10">
        <motion.div
  ...
  return (
    ...
    >
      {/* Header with Stats */}
      <motion.div variants={itemVariants} className="max-w-5xl mx-auto mb-6 pt-5">
          ...
          >
            ...
            >
              ...
              <div className="space-y-6">
                ...
                {!selectedNotification.read && (
                  <div className="flex justify merkezi pt-4">
                    <motion.button
