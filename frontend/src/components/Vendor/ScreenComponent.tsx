...
interface SeatComponentProps {
  seat: Seat | null; // Allow null to handle edge cases
...
const SeatComponent: React.FC<SeatComponentProps> = ({ seat, onClick, onContextMenu, selected }) => {
  if (!seat) {
    ...
    return <div className="w-8 h-8 bg-gray-300 rounded-t-lg flex items-center justify-center">?</div>; // Fallback UI
  }

  const seatTypeInfo = SEAT_TYPES[seat.type] || SEAT_TYPES.REGULAR; // Fallback to REGULAR if type is invalid
