...
export class ForgotPasswordUpdateUseCase implements IForgotPasswordUpdateUseCase {
    ...
    async execute(email: string, password: string): Promise<void> {1
        ...
        try {
            ...
            throw new CustomError(ERROR_MESSAGES.GENERAL.FAILED_TO_UPDATE_PASSWORD, HttpResCode.INTERNAL_SERVER_ERROR);
        }
    }
}